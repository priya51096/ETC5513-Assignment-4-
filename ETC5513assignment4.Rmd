...
```{r topdestinations, echo = FALSE, message = FALSE}
...
top_10 <- whomigrates %>%  mutate(
  ...
  group_by(country) %>% summarise(Immigrant_Count=sum(number)) %>% arrange(desc(Immigrant_Count)) %>% head(10)  
...
```{r gendergap, echo = FALSE, message = FALSE, fig.cap="Gender difference in migrant population in countries" , fig.width=10}
...
```{r agegap, echo = FALSE, message = FALSE, fig.cap="Gender difference in migrant population in countries across age groups" , fig.width=10}
agegap <- whomigrates  %>% 
  mutate(
         sex=as.factor(sex),
         oecd=as.factor(oecd),#oecd resident
