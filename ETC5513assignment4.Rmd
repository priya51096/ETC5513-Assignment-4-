...
\section*{Introduction}
   This dataset provides information about OECD coutries. each file includes a number of core variables (detailed country of birth, education and sex). So our goals is to study immigrant in OECD countries and use these core variables to analyse the situation of immigrants.
...
```{r}
GGM1 <- GGM %>% mutate(unemployrateM = unemployed/(employed + inactive + unemployed)) %>% select(country,unemployrateM)
GGM1
```
...
```{r}
GGF1 <- GGF %>% mutate(unemployrateF = unemployed/(employed + inactive + unemployed)) %>% select(country,unemployrateF)
GGF1
```
```{r}
...
```{r GAP, fig.width=11, fig.height=10, fig.cap ="unemployment gender gap"}
ggplot(filter(GGMF, country!="LUX", country != "KOR"), aes(x = country, y = unemployrateGAP)) +  
  geom_col() 
```
...
```{r ed_un, fig.width=11, fig.height=10, fig.cap ="unemployment in different education level"}
ggplot(filter(C4,country!="LUX", country != "KOR"), aes(x = educationlevel, y = unemployRATE)) + geom_col() +  facet_wrap(~ country)
```
