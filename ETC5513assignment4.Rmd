...
```{r}
GGMF %>% head(10) %>% kable(caption = "unemployment gendergap")
```
...
```
   ...
   I made this \ref{fig:ed_un} to show unemployment on different education level in different countries, generally speaking, most have lower unemployment rate compared to low education level groups, but this is not the case in TUR,PRT,RGC AND ITA.
