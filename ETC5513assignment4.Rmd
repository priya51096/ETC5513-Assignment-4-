...
ggplot(filter(GGMF), aes(x = unemployrateGAP, y = country, color = country, fill = country)) +  
  geom_col()
```
...
ggplot(filter(C4), aes(x = educationlevel, y = unemployRATE, color = educationlevel, fill = educationlevel)) + geom_col() +  facet_wrap(~ country)
```
